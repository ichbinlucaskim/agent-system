...
설정은 environment에서 오며, 최초 사용 시점이 아니라 기동 시점에 검증됩니다. 문제없이 시작한 뒤 한 시간 후 첫 요청에서 실패하는 프로세스는, 시작을 거부하며 어떤 변수가 없는지 말해 주는 프로세스보다 훨씬 진단하기 어렵습니다. `load_config`와 CLI는 `ANTHROPIC_API_KEY` 없이 기동을 거부합니다. smoke test는 키가 없을 때 echo core로 떨어져 HTTP plumbing을 오프라인으로 증명할 수 있습니다 — 그 fallback은 lab 하네스용이며 실제 배포의 기동 검증을 느슨하게 하는 것이 아닙니다.
...
6. 필요한 environment variable을 모듈 docstring에 문서화하고, `load_config`/CLI가 그것들 없이 기동을 거부하는지 확인하세요. (smoke는 오프라인에서 echo core를 쓸 수 있지만, 실제 프로세스의 기동 검증을 대체하지는 않습니다.)
...
설정, 인자 파싱, HTTP 상태 처리, smoke test는 오프라인으로 테스트됩니다. 통과했다면 필수 변수 누락이 기동 시점에 이름과 함께 보고되고, 잘못된 `AGENT_PORT`가 거절되며, API key가 없어도 `/health`가 core를 부르지 않고 200을 반환하고, 잘못된 형식의 JSON과 `question` 누락이 트레이스백 대신 400을 반환하며, CLI가 core 실패 시 종료 코드 1·설정 실패 시 2를 반환하고, 두 진입점이 같은 핵심 함수를 호출하며, 모듈 docstring이 environment 변수를 나열하고, `smoke_test`가 실제로 health/malformed/valid를 점검한다(health가 깨지면 실패)는 뜻입니다.
