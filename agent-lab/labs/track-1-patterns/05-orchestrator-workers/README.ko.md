...
다만 그 검증이 할 수 없는 일을 알아 두세요. lab 03에서는 classifier가 닫힌 집합에서 골랐기 때문에 답을 집합과 대조해 틀렸다는 것을 알 수 있었습니다. 계획은 대조할 대상이 없는 열린 출력이라서, 검증은 형태만 볼 수 있을 뿐 타당성은 결코 볼 수 없습니다. "다른 하위 작업들의 결과를 종합하라"라고 적힌 하위 작업은 id가 있고 설명이 있고 중복도 아니어서 모든 검사를 통과하지만, 자신이 읽겠다고 한 하위 작업들과 동시에 실행되므로 수행 자체가 불가능합니다. 이런 항목에 대한 유일한 방어선은 planner의 prompt이며, 그래서 규칙은 거기에 있어야 하고 "independent"라는 단어가 알아서 덮어 주기를 기대하는 대신 명시적으로 적을 가치가 있습니다.
...
1. `plan`을 구현하세요. 작업을 읽고 각각 id와 설명을 가진 하위 작업 리스트를 구조화된 데이터로 반환하는 lead 호출 하나입니다. planner가 지켜야 할 규칙을 `PLAN_RULES`에 정의하고 system prompt에 넣으세요. 각 하위 작업이 홀로 성립할 것, 다른 하위 작업들을 종합하는 하위 작업을 만들지 말 것, id는 유일할 것.
2. `validate_plan`을 구현하세요. 필드가 빠진 항목을 거부하고, 중복을 제거하고, 겹치는 id를 제거하고, `max_subtasks`로 잘라냅니다. 생성된 계획은 신뢰할 수 없는 입력입니다.
...
계획 검증과 orchestration의 control flow는 stub planner를 사용해 오프라인으로 테스트됩니다. 통과했다면 잘못된 형식의 계획이 실행되지 않고 거부되며, 하위 작업 상한이 지켜지고, 겹치는 id가 검증을 통과하지 못하며, planner가 지켜야 할 규칙을 전달받고, 실패한 worker가 실행 전체를 중단시키지 않으며, 반환 결과에 계획이 포함되어 비싼 실행을 설명할 수 있다는 뜻입니다.

## 더 나아가기

- 같은 작업을 손으로 쓴 계획으로 lab 04의 sectioning처럼 실행하고, 생성된 계획과 품질 및 비용을 비교하세요.
- planner prompt에서 `PLAN_RULES`를 빼고 계획을 대여섯 번 생성한 뒤, 다른 하위 작업에 의존하는 항목이 몇 번의 실행에 나타나는지 세어 보세요. 어떤 실행에서 나오는지도 보세요. 상한을 꽉 채운 계획이 어긋날 여지가 가장 큽니다.
