...
Use a judge only for what cannot be asserted: tone, faithfulness to a source, whether an explanation is actually helpful. The judge is itself a model call and carries all the variance you are trying to measure, so give it a rubric with specific, independently gradeable criteria and ask for structured output rather than a verdict in prose.
...
Keep the failures. A report that says eighty percent is not actionable; a report that names the four cases that failed and shows what they returned is. Store the outputs so a regression can be diagnosed without re-running the whole suite.
...
4. Implement `run_suite`: run every case `runs` times, collect deterministic and judge results, and keep the raw outputs.
5. Implement `format_report`: print a per-case pass rate, mark cases between 0 and 1 as flaky, and list the failures with their outputs.
6. Change one prompt, re-run the suite, and compare the reports. That comparison is the point of the entire lab.
...
The suite machinery is tested offline with stubbed outputs. Passing means a deterministic check reports its reason on failure, the pass rate is computed across runs rather than from the last run, a case that passes intermittently is marked flaky, the report names every failing case, and a judge response that does not parse is recorded as an error instead of scoring zero.
